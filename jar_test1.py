...
class ConfigManager:
    ...
    def load_coagulants(self):
        try:
            ...
            # S'assurer que "Aucun" est le premier élément
            if coagulants and coagulants[0]["nom"] != "Aucun":
                # Chercher "Aucun" et le mettre en premier
                for i, coag in enumerate(coagulants):
                    if coag["nom"] == "Aucun":
                        coagulants.insert(0, coagulants.pop(i))
                        break
                else:
                    # Si "Aucun" n'existe pas, l'ajouter en premier
                    coagulants.insert(0, {
                        "nom": "Aucun",
                        "dilution": 1.0,
                        "densite": 1.0,
            ...
            # Base de coagulants du commerce avec "Aucun" en premier
            return [
                {
                    "nom": "Aucun",
                    "dilution": 1.0,
    ...
    def load_floculants(self):
        try:
            ...
            # Base de floculants du commerce avec "Aucun" en premier
            return [
                {
                    "nom": "Aucun",
                    "type": "Liquide",
    ...
    def load_parametres(self):
        try:
            ...
            return {
                ...
                "parametres_selectionnes": ["Turbidité", "pH", "DCO"]  # DCO remplace Couleur par défaut
            }
...
def generer_rapport_pdf(date_test, operateur, site_prelevement, type_eau, volume_echantillon, 
    ...
    for combinaison, df in tableau_essais.items():
        ...
        table_data = [["Essai", "Coag (ppm com.)", "Coag (ppm actif)", "Floc (ppm com.)", "Floc (ppm actif)", 
                      "DCO e", "DCO s", "pH e", "pH s", "V boue", "Abattement %"]]
        
        for i, row in df.iterrows():
            table_data.append([
                str(int(row['Essai'])),
                f"{row['Coagulant_ppm_com']:.2f}",
                f"{calculer_ppm_actif(row['Coagulant_ppm_com'], next((c for c in coagulants_config if c['nom'] in combinaison), {'matiere_active': 0})['matiere_active']):.2f}" if row['Coagulant_ppm_com'] > 0 else "0.00",
                f"{row['Floculant_ppm_com']:.2f}",
                f"{calculer_ppm_actif(row['Floculant_ppm_com'], next((f for f in floculants_config if f['nom'] in combinaison), {'matiere_active': 0})['matiere_active']):.2f}" if row['Floculant_ppm_com'] > 0 else "0.00",
                f"{row['DCO_entree']:.2f}",
                f"{row['DCO_sortie']:.2f}",
                f"{row['pH_entree']:.2f}",
                f"{row['pH_sortie']:.2f}",
        ...
        # Créer le tableau
        table = Table(table_data, colWidths=[0.5*inch, 1*inch, 1*inch, 1*inch, 1*inch, 0.7*inch, 0.7*inch, 0.5*inch, 0.5*inch, 0.7*inch, 1*inch])
            ...
            if not coagulants_a_afficher:
                ...
                for i, coag in enumerate(coagulants_a_afficher):
                    ...
                    with st.expander(f"🔬 {coag['nom']}", expanded=False):
                        ...
                        st.write(f"**Volume pour 1 ppm:** {volume_ppm:.2f} mL/kg")
...
def main():
    ...
    with st.sidebar:
        ...
        if "UV254" in parametres_selectionnes:
            caracteristiques['uv254_entree'] = st.number_input("UV254 initial (abs)", min_value=0.00, value=0.1, step=0.001, format="%.2f")
    ...
    # Onglets principaux
    tab1, tab2, tab3, tab4 = st.tabs(["🔄 Combinaisons", "📊 Saisie Essais", "📈 Graphiques", "📄 Rapport Complet"])
    ...
    with tab3:
        st.markdown('<h2 class="section-header">Graphiques des Résultats</h2>', unsafe_allow_html=True)
        
        # Récupérer les données de la base pour les graphiques
        db_manager = DatabaseManager()
        ...
        if not mesures_db.empty:
            ...
            if not mesures_courantes.empty:
                ...
                with col1:
                    fig_dco = px.line(
                        mesures_courantes, x='coagulant_ml', y='abattement', color='combinaison',
                        title='Abattement DCO en fonction du dosage de coagulant',
                        markers=True
                    )
                    st.plotly_chart(fig_dco, use_container_width=True)
                
                with col2:
                    fig_boue = px.bar(
                        mesures_courantes, x='essai', y='v_boue', color='combinaison',
                        title='Volume de boue par essai et combinaison',
                        barmode='group'
                    )
                    st.plotly_chart(fig_boue, use_container_width=True)
...
"""
                ...
                for combinaison, df in st.session_state.tableau_essais.items():
                    rapport += f"\n### {combinaison}\n"
                    rapport += "| Essai | Coag (ppm com.) | Coag (ppm actif) | Floc (ppm com.) | Floc (ppm actif) | DCO e | DCO s | pH e | pH s | V boue | Abattement % |\n"
                    rapport += "|-------|-----------------|------------------|-----------------|------------------|-------|-------|------|------|--------|-------------|\n"
                    
                    for i, row in df.iterrows():
                        coag_actif = calculer_ppm_actif(row['Coagulant_ppm_com'], next((c for c in coagulants_config if c['nom'] in combinaison), {'matiere_active': 0})['matiere_active']) if row['Coagulant_ppm_com'] > 0 else 0
                        floc_actif = calculer_ppm_actif(row['Floculant_ppm_com'], next((f for f in floculants_config if f['nom'] in combinaison), {'matiere_active': 0})['matiere_active']) if row['Floculant_ppm_com'] > 0 else 0
                        
                        rapport += f"| {int(row['Essai'])} | {row['Coagulant_ppm_com']:.2f} | {coag_actif:.2f} | {row['Floculant_ppm_com']:.2f} | {floc_actif:.2f} | {row['DCO_entree']:.2f} | {row['DCO_sortie']:.2f} | {row['pH_entree']:.2f} | {row['pH_sortie']:.2f} | {row['V_boue']:.2f} | {row['Abattement']:.2f}% |\n"
